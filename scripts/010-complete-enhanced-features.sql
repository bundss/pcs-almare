-- Complete enhanced features script
-- This includes everything that should have been in script 005
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'approval_status') THEN
        ALTER TABLE user_profiles ADD COLUMN approval_status VARCHAR(20) DEFAULT 'approved' CHECK (approval_status IN ('pending', 'approved', 'rejected'));
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'user_profiles' AND column_name = 'approved_at') THEN
        ALTER TABLE user_profiles ADD COLUMN approved_at TIMESTAMP WITH TIME ZONE;
    END IF;
END $$;

-- Add completion status to PCS entries if they don't exist
DO $$ 
BEGIN
    -- Add is_completed column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pcs_entries' AND column_name = 'is_completed') THEN
        ALTER TABLE pcs_entries ADD COLUMN is_completed BOOLEAN DEFAULT FALSE;
    END IF;
    
    -- Add completed_at column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pcs_entries' AND column_name = 'completed_at') THEN
        ALTER TABLE pcs_entries ADD COLUMN completed_at TIMESTAMP WITH TIME ZONE;
    END IF;
    
    -- Add completed_by column if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pcs_entries' AND column_name = 'completed_by') THEN
        ALTER TABLE pcs_entries ADD COLUMN completed_by UUID;
    END IF;
    
    -- Add mindmap positioning columns if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pcs_entries' AND column_name = 'mindmap_x') THEN
        ALTER TABLE pcs_entries ADD COLUMN mindmap_x FLOAT DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'pcs_entries' AND column_name = 'mindmap_y') THEN
        ALTER TABLE pcs_entries ADD COLUMN mindmap_y FLOAT DEFAULT 0;
    END IF;
END $$;
...
-- Create public access tokens table if it doesn't exist
...
-- Create pending user notifications table if it doesn't exist
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        ...
        VALUES (NEW.patient_id, NEW.id, NULL, 'created', 'Nova entrada PCS criada: ' || NEW.title);
        RETURN NEW;
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        IF OLD.is_completed != NEW.is_completed THEN
            ...
            VALUES (NEW.patient_id, NEW.id, NULL, 
        ...
        IF OLD.title != NEW.title THEN
            INSERT INTO pcs_change_log (patient_id, entry_id, user_id, action, field_name, old_value, new_value, description)
            VALUES (NEW.patient_id, NEW.id, NULL, 'updated', 'title', OLD.title, NEW.title, 'Título alterado');
        END IF;
        
        -- Log description changes
        IF COALESCE(OLD.description, '') != COALESCE(NEW.description, '') THEN
            INSERT INTO pcs_change_log (patient_id, entry_id, user_id, action, field_name, old_value, new_value, description)
            VALUES (NEW.patient_id, NEW.id, NULL, 'updated', 'description', OLD.description, NEW.description, 'Descrição alterada');
        END IF;
        
        RETURN NEW;
    ELSIF TG_OP = 'DELETE' THEN
        INSERT INTO pcs_change_log (patient_id, entry_id, user_id, action, description)
        VALUES (OLD.patient_id, OLD.id, NULL, 'deleted', 'Entrada PCS removida: ' || OLD.title);
        RETURN OLD;
    END IF;
    RETURN NULL;
END;
...
FROM patients
WHERE id NOT IN (SELECT COALESCE(patient_id, '00000000-0000-0000-0000-000000000000'::uuid) FROM patient_public_tokens)
ON CONFLICT DO NOTHING;

-- Update existing user profiles to approved status
UPDATE user_profiles 
SET approval_status = 'approved', approved_at = NOW()
WHERE approval_status IS NULL;

-- Show what was created
SELECT 'Enhanced features installed successfully!' as result;
